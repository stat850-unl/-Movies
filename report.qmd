---
title: "Stat 850 Project Report: Artists in the US"
format: html
editor: visual
---
...
Imputation techniques, facilitated by the **`dplyr`** package in R, were applied to handle missing values. Additionally, anomalies within categorical variables were identified and addressed through the use of the **`filter`** and **`mutate`** functions. Variables were transformed as necessary to harmonize units or scales. The **`mutate`** function in the **`dplyr`** package played a crucial role in ensuring a standardized and coherent data-set for subsequent analyses. There are lots of other packages have been used in this project as the following:
...
The project employed a diverse set of exploratory data analysis techniques to uncover insights into the dataset.

#### Distribution of Artist Types:

Visualization techniques, including heatmaps, bar plots, and pie charts, were used to illustrate the distribution of artist types across different states. Both Python and R programming languages were employed to create diverse visualizations.

#### Location Quotient Analysis:

Average location quotients were calculated for each state, and the distribution was analyzed using histograms and box plots. Anomalies, such as the outlier in the District of Columbia, were identified and flagged for further investigation.

#### Correlation Analysis:

Pearson correlation coefficients \[12\] were computed to analyze the relationship between the percentage of artists in a state (**`artists_share`**) and the state's number of workers (**`all_workers_n`**).

#### Racial Diversity:

Racial diversity among artists across states was visualized using bar plots and facet wraps in ggplot2. The distribution of Asian artists was explored in-depth, identifying top states based on artist count and population percentage.

### **Shiny Apps for Interactive Exploration:**

Interactive Shiny apps were developed to facilitate dynamic exploration of artist distributions. Users could interactively choose specific races, set the number of top states for analysis, and observe the distribution in real-time.

### **Statistical Analyses:**

Statistical analyses were conducted using a combination of R and Python libraries, including **`scipy`** and **`stats`**. Calculations included average location quotient, correlation coefficients, and racial compositions
...
Consider the specific goals of our analysis and the characteristics of our dataset when choosing a visualization method. Based on what we learnt in the class, we want to create one shiny graph (which will not reflect in this report. However, you can check it in the [Project](Project.qmd) to answer this question. Here we just show one state as the example
...
# Filter the data for Nebraska and group by type
nebraska_data <- artist %>%
  filter(state == "Nebraska") %>%
  group_by(type) %>%
  summarise(total_artists_n = sum(artists_n))  # Sum of artists_n for each type

# Bar Plot for Nebraska without legend
bar_plot_nebraska <- ggplot(nebraska_data, aes(x = type, y = total_artists_n, fill = type)) +
  geom_bar(stat = "identity") +
  labs(title = "Number of Artists for Each Artist Type in Nebraska",
       x = "Artist Type",
       y = "Number of Artists") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
...
# Print the bar plot for Nebraska
print(bar_plot_nebraska)
...
Taking Nebraska as an illustrative example, the bar plot unveils notable differences in the distribution of artists across various types. Particularly noteworthy is the "designers" category, showcasing a significant total surpassing 4000 artists. In contrast, the "actors" category exhibits a relatively lower total, amounting to fewer than 500 artists. This visual representation offers a lucid portrayal of the relative magnitudes of artist types in Nebraska, underscoring the diversity and prevalence of distinct artistic vocations.
...
In this part, we calculate the correlation using R. The correlation between two variables, X and Y, is calculated using the Pearson correlation coefficient (r) \[12\], which is defined by the following formula:
...
Baes on the question 5, we can get that the population and the The last question, and it can be articulated as follows:
...
# Merge the two datasets
merged_data_hispanic <- merge(artist_composition_hispanic, population_composition_hispanic, by = "state", all = TRUE)

# Sort merged data by artist_count in descending order
sorted_data_hispanic <- merged_data_hispanic[order(-merged_data_hispanic$artist_count), ]

# Function to get top N states for Hispanic race
get_top_states_hispanic <- function(n = 10) {
  top_states_hispanic <- head(sorted_data_hispanic, n)
  return(top_states_hispanic)
}

# Example: Get top 10 states for the Hispanic race
top_states_hispanic <- get_top_states_hispanic(n = 10)

# Print the results
cat("Top 10 states for Hispanic race:\n")
print(top_states_hispanic)
cat("\n")

# Plot pie chart for population percentage
race_data_hispanic <- top_states_hispanic
pie(race_data_hispanic$population_percentage, labels = race_data_hispanic$state,
    main = "Pie Chart of Population Percentage for Hispanic race")

```

The Shiny app in [Project](Project.qmd) provides an insightful and interactive platform to examine the distribution of artists across various racial categories within U.S. states. Users can dynamically explore artist counts and population percentages for different races, gaining valuable insights into the artistic landscape's diversity. The app accommodates not only the analysis of Asian artists, where California emerges as a leader with the highest count, but also extends this functionality to other racial categories. Users have the flexibility to select a specific race, determine the number of top states to analyze, and promptly observe the corresponding distribution. This comprehensive tool facilitates a nuanced exploration of racial diversity within the artist community, fostering a deeper understanding of the geographic concentrations and proportions of artists from various racial backgrounds across the United States.

In this report, take "Hispanic artist" as one example. In exploring the racial distribution of Hispanic artists across the "Top 10 states for the Hispanic race," California emerges as a significant focal point, exhibiting the highest count and percentage of Hispanic artists at 29.42%. Notably, this closely mirrors the state's Hispanic population percentage of 29.17%. Following closely is Texas, where a substantial count of 37,725 Hispanic artists contributes to a percentage of 14.87%, slightly below the state's Hispanic population percentage of 20.38%. Florida reveals a concentrated presence, with a Hispanic artist percentage (13.29%) surpassing its population percentage (10.75%). New York, known for its diverse artistic scene, presents a noteworthy 10.74% Hispanic artist percentage, exceeding the state's Hispanic population percentage of 7.10%. This examination sheds light on nuanced regional dynamics and provides valuable insights into the representation of Hispanic artists in the United States, emphasizing the variations across these top-ranking states.
...
Future research should undertake a comprehensive exploration of the economic and cultural factors that underlie the distribution and location quotients of artists across various states. This investigation aims to unravel the intricate influences shaping the concentration of artistic communities in different regions. Additionally, there is a need to delve into the policy implications for states exhibiting lower location quotients, focusing on strategies to promote and bolster the artistic community. Another crucial facet involves conducting an in-depth analysis of long-term trends in the distribution of artists. This examination should encompass factors like migration patterns and the evolving cultural landscapes, providing valuable insights into the dynamic forces that influence the geographical presence of artists over extended periods.
...
2.  https://remotes.r-lib.org/
3.  Jak, Suzanne, Terrence D. Jorgensen, Mathilde GE Verdam, Frans J. Oort, and Louise Elffers. "Analytical power calculations for structural equation modeling: A tutorial and Shiny app." Behavior Research Methods 53 (2021): 1385-1406.
4.  Wickham, Hadley. "ggplot2." Wiley interdisciplinary reviews: computational statistics 3, no. 2 (2011): 180-185.
5.  Wickham, Hadley, Mara Averick, Jennifer Bryan, Winston Chang, Lucy D'Agostino McGowan, Romain François, Garrett Grolemund et al. "Welcome to the Tidyverse." Journal of open source software 4, no. 43 (2019): 1686.
6.  https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html
7.  Wickham, Hadley, Jim Hester, Romain Francois, Jennifer Bryan, Shelby Bearrows, and P. B. C. Posit. "Package 'readr'." Read Rectangular Text Data. Available online: https://cran. r-project. org/web/packages/readr/readr. pdf (accessed on 28 June 2022) (2023).
8.  https://www.r-project.org/
9.  Reback, Jeff, Wes McKinney, Joris Van Den Bossche, Tom Augspurger, Phillip Cloud, Adam Klein, Simon Hawkins et al. "pandas-dev/pandas: Pandas 1.0. 5." Zenodo (2020).
10. Virtanen, Pauli, Ralf Gommers, Travis E. Oliphant, Matt Haberland, Tyler Reddy, David Cournapeau, Evgeni Burovski et al. "SciPy 1.0: fundamental algorithms for scientific computing in Python." Nature methods 17, no. 3 (2020): 261-272.
11. Tosi, Sandro. Matplotlib for Python developers. Packt Publishing Ltd, 2009.
12. Cohen, Israel, Yiteng Huang, Jingdong Chen, Jacob Benesty, Jacob Benesty, Jingdong Chen, Yiteng Huang, and Israel Cohen. "Pearson correlation coefficient." Noise reduction in speech processing (2009): 1-4.
